  ...
  return (
    <AnimatedPage style={S.page}>
      <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }} style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 800, fontSize: '1.75rem', color: theme.textPrimary }}>Implementing Agency Benchmarking</h1>
        <p style={{ color: theme.textMuted, fontSize: '0.85rem', marginTop: 4 }}>Outcome (e) — Comparative Analytics & Reliability Index across Central Sector Infrastructure Agencies</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <div style={S.cityCard}>
          ...
          <div style={{ ...S.cityLeft, position: 'relative', zIndex: 1 }}>
            ...
            <div>
              <div style={S.cityLabel}>PORTFOLIO RELIABILITY INDEX</div>
              <div style={S.cityScore}><CountUp target={avgTrust} duration={1.5} /><span style={S.cityMax}>/100</span></div>
              <div style={S.cityNote}>Based on 1,775 projects · 17 Ministries · CUF Monthly Reconciliation</div>
            </div>
          </div>
          
          <div style={{ ...S.cityMetrics, position: 'relative', zIndex: 1 }}>
            {[['Timeliness Rate', 78], ['Cost Discipline', 86], ['Milestone Accuracy', 74], ['CUF Compliance', 92]].map(([l, v], idx) => (
      ...
      <motion.h2 initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 700, fontSize: '1.1rem', color: theme.textPrimary, marginBottom: '1rem' }}>
        Implementing Agency & Project Reliability Rankings
      </motion.h2>
