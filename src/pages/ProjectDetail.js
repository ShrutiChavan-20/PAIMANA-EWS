        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <FloatingCard style={S.main} glowColor={`${statusColor[p.status]}20`}>
            ...
            <div style={S.mainInner}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.2rem'}}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '0.68rem', background: 'rgba(99,102,241,0.15)', color: '#6366f1', padding: '3px 8px', borderRadius: 6, fontWeight: 700 }}>
                      {p.projectCode || `PAI-${p.id}`}
                    </span>
                    <div style={S.catTag}>{p.category}</div>
                    <span style={{ fontSize: '0.72rem', background: 'rgba(14,165,233,0.1)', color: '#0ea5e9', padding: '2px 8px', borderRadius: 6, fontWeight: 600 }}>
                      {p.ministry || 'Central Sector'}
                    </span>
                  </div>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:12,marginTop:6,flexWrap:'wrap'}}>
                    <div style={{display:'flex',alignItems:'center',gap:4}}>
                      ...
                      <span style={{fontSize:'0.85rem',color:theme.textMuted,fontWeight:500}}>{p.state || p.location}</span>
                    </div>
                    <div style={{fontSize:'0.8rem',color:'#6366f1',fontWeight:600}}>
                      Agency: {p.agency || p.contractor}
                    </div>
                  </div>
                </div>
              ...
              {/* ── PAIMANA Financial & Timeline Grid ── */}
              <StaggerContainer style={S.infoGrid}>
                {[
                  ['Original Cost', p.originalCostCr ? `₹${Math.round(p.originalCostCr).toLocaleString()} Cr` : p.budget, IndianRupee],
                  ['Revised Cost', p.revisedCostCr ? `₹${Math.round(p.revisedCostCr).toLocaleString()} Cr` : p.budget, IndianRupee],
                  ['Cumulative Spend', p.spentCr ? `₹${Math.round(p.spentCr).toLocaleString()} Cr` : p.spent, IndianRupee],
                  ['Revised DoC', p.revisedDoC || p.endDate, Calendar]
                ].map(([l,v,Icon])=>(
                  <StaggerItem key={l}>
              ...
              {/* ── AI Early Warning & Overrun Analysis Card ── */}
              <div style={{
                marginTop: '1.2rem',
                padding: '1.2rem',
                borderRadius: 14,
                background: isDark ? 'rgba(15,23,42,0.75)' : 'rgba(241,245,249,0.7)',
                border: `1px solid ${p.costOverrunCr > 0 || p.delayMonths > 0 ? 'rgba(239,68,68,0.3)' : 'rgba(16,185,129,0.3)'}`,
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <AlertTriangle size={15} color={p.costOverrunCr > 0 ? '#ef4444' : '#10b981'} />
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '0.72rem', fontWeight: 800, color: theme.textPrimary, letterSpacing: '0.05em' }}>
                      PAIMANA AI OVERRUN & RISK DIAGNOSTIC
                    </span>
                  </div>
                  <span style={{
                    ...
                    padding: '2px 8px',
                    borderRadius: 6,
                    background: p.status === 'critical' ? 'rgba(239,68,68,0.2)' : p.status === 'delayed' ? 'rgba(245,158,11,0.2)' : 'rgba(16,185,129,0.2)',
                    color: statusColor[p.status],
                    fontWeight: 700
                  }}>
                    RISK SCORE: {p.riskScore || (100 - p.trustScore)} / 100
                  </span>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: 12 }}>
                  <div style={{ padding: '8px 10px', background: isDark ? 'rgba(0,0,0,0.25)' : '#ffffff', borderRadius: 8 }}>
                    <div style={{ fontSize: '0.62rem', color: theme.textMuted, fontFamily: "'JetBrains Mono',monospace" }}>COST ESCALATION</div>
                    <div style={{ fontSize: '0.92rem', fontWeight: 700, color: p.costOverrunCr > 0 ? '#ef4444' : '#10b981', marginTop: 2 }}>
                      {p.costOverrunCr > 0 ? `+₹${Math.round(p.costOverrunCr).toLocaleString()} Cr (${p.costOverrunPct}%)` : '₹0 (On Budget)'}
                    </div>
                  </div>

                  <div style={{ padding: '8px 10px', background: isDark ? 'rgba(0,0,0,0.25)' : '#ffffff', borderRadius: 8 }}>
                    <div style={{ fontSize: '0.62rem', color: theme.textMuted, fontFamily: "'JetBrains Mono',monospace" }}>SCHEDULE SLIPPAGE</div>
                    <div style={{ fontSize: '0.92rem', fontWeight: 700, color: p.delayMonths > 0 ? '#f59e0b' : '#10b981', marginTop: 2 }}>
                      {p.delayMonths > 0 ? `+${p.delayMonths} Months Delay` : 'Zero Slippage'}
                    </div>
                  </div>

                  <div style={{ padding: '8px 10px', background: isDark ? 'rgba(0,0,0,0.25)' : '#ffffff', borderRadius: 8 }}>
                    <div style={{ fontSize: '0.62rem', color: theme.textMuted, fontFamily: "'JetBrains Mono',monospace" }}>SPEND VS PROGRESS GAP</div>
                    <div style={{ fontSize: '0.92rem', fontWeight: 700, color: p.divergenceGap < -10 ? '#ef4444' : '#10b981', marginTop: 2 }}>
                      {p.divergenceGap !== undefined ? `${p.divergenceGap > 0 ? '+' : ''}${p.divergenceGap}%` : 'Synchronized'}
                    </div>
                  </div>
                </div>

                <div style={{ fontSize: '0.72rem', color: theme.textSecondary, lineHeight: 1.5 }}>
                  <strong>AI Early Warning Note:</strong> {p.delayMonths > 24 ? 'Severe milestone slippage exceeding 2 years. Primary intervention recommended on land acquisition and inter-agency RoW clearances.' : p.delayMonths > 0 ? 'Moderate schedule drift detected. Expenditure velocity remains moderate; watch milestone completion pace.' : 'Project executing within sanctioned parameters with healthy financial burn rate.'}
                </div>
              </div>
              
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} style={S.progressSection}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
                  <span style={{fontFamily:"'Outfit',sans-serif",fontWeight:700,fontSize:'0.95rem',color:theme.textPrimary}}>Physical Progress</span>
                  <span style={{fontFamily:"'Outfit',sans-serif",fontWeight:800,color:statusColor[p.status],fontSize:'1.1rem'}}><CountUp target={p.progress || p.physicalProgress} suffix="%" /></span>
                </div>
                <div style={{height:12,background:isDark?'rgba(0,0,0,0.2)':'rgba(0,0,0,0.05)',borderRadius:6,overflow:'hidden'}}>
                  <motion.div initial={{ width: 0 }} animate={{ width: `${p.progress || p.physicalProgress}%` }} transition={{ duration: 1.5, ease: "easeOut" }} style={{height:'100%',background:statusColor[p.status],borderRadius:6,boxShadow:`0 0 12px ${statusColor[p.status]}50`}}/>
                </div>
                <div style={{display:'flex',justifyContent:'space-between',marginTop:6}}>
                  <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'0.65rem',color:theme.textMuted,fontWeight:600}}>TARGET DOC: {p.targetDoC || p.startDate}</span>
